...
function sortearLouvores(qtd) {
    ...
    while (indicesSorteados.size < qtd) {
        const indiceAleatorio = Math.floor(Math.random() * listaDeLouvores.length);
        indicesSorteados.add(indiceAleatorio);
    }

    return Array.from(indicesSorteados).map(index => listaDeLouvores[index]);
}
